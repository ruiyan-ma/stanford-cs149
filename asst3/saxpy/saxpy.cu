...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double knlStartTime = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();
    double knlEndTime = CycleTimer::currentSeconds();
    ...
    double knlDuration = knlEndTime - knlStartTime;
    printf("GPU BW: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * knlDuration, GBPerSec(totalBytes, knlDuration));
