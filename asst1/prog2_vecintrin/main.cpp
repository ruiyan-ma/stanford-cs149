...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    __cs149_mask maskStore = _cs149_init_ones();
    __cs149_mask maskGtZero, maskClamp;

    int tailNum = N % VECTOR_WIDTH;
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        result = _cs149_vset_float(1.f);
        _cs149_vload_float(val, values + i, maskAll);
        _cs149_vload_int(exp, exponents + i, maskAll);

        // while count > 0, result *= val
        _cs149_vgt_int(maskGtZero, exp, zero, maskAll);
        while (_cs149_cntbits(maskGtZero) > 0) {
            _cs149_vmult_float(result, result, val, maskGtZero);
            _cs149_vsub_int(exp, exp, one, maskGtZero);
            _cs149_vgt_int(maskGtZero, exp, zero, maskAll);
        }

        // clamp result value to 9.999999f
        _cs149_vgt_float(maskClamp, result, clampVal, maskAll);
        _cs149_vmove_float(result, clampVal, maskClamp);

        // store partial result for last vector
        if (i == N - tailNum) maskStore = _cs149_init_ones(tailNum);
        _cs149_vstore_float(output + i, result, maskStore);
    }
}
...
float arraySumVector(float* values, int N) {
    ...
    __cs149_mask maskAll = _cs149_init_ones();
    __cs149_vec_float val, result;
    result = _cs149_vset_float(0.f);

    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        _cs149_vload_float(val, values + i, maskAll);
        _cs149_vadd_float(result, result, val, maskAll);
    }

    int niter = VECTOR_WIDTH;
    while (niter / 2 > 0) {
